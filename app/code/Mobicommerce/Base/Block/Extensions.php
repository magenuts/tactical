...
namespace Mobicommerce\Base\Block;
...
class Extensions extends \Magento\Config\Block\System\Config\Form\Fieldset
{
    ...
    /**
     * @var \Mobicommerce\Base\Helper\Module
     */
    ...
    public function __construct(
        ...
        \Mobicommerce\Base\Helper\Module $moduleHelper,
    ...
    public function render(AbstractElement $element)
    {
        ...
        foreach ($modules as $moduleName) {
            if (strstr($moduleName, 'Mobicommerce_') === false
                || $moduleName === 'Mobicommerce_Base'
    ...
    protected function _getFieldHtml($fieldset, $moduleCode)
    {
        ...
        $moduleName = $this->_replaceMobicommerceText($moduleName);
        $status =
             '<a target="_blank">
                <img src="'. $this->getViewFileUrl('Mobicommerce_Base::images/ok.gif') . '" title="' . __("Installed") . '"/>
             </a>';
        ...
        if ($allExtensions && isset($allExtensions[$moduleCode])) {
            ...
            $url     = $ext['url'];
            $name    = $ext['name'];
            $name = $this->_replaceMobicommerceText($name);
            $lastVer = $ext['version'];
            ...
            if (version_compare($currentVer, $lastVer, '<')) {
                $status =
                    '<a href="' . $url . '" target="_blank">
                        <img src="' . $this->getViewFileUrl('Mobicommerce_Base::images/update.gif') .
        ...
        if ($this->_scopeConfig->getValue('advanced/modules_disable_output/' . $moduleCode)) {
            ...
            $status =
                '<a' . $href . ' target="_blank">
                    <img src="' . $this->getViewFileUrl('Mobicommerce_Base::images/bad.gif') .
     ...
     * @return mixed
     */
    protected function _replaceMobicommerceText($moduleName)
    {
        $moduleName = str_replace('for Magento 2', '', $moduleName);
        $moduleName = str_replace('by Mobicommerce', '', $moduleName);
